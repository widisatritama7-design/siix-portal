...
<div class="max-w-3xl mx-auto px-4">
    ...
    <div class="mb-6 text-center">
        ...
        <p class="text-gray-600 mt-1">Scan PCB barcode for final inspection</p>
    </div>
        ...
        <div class="p-6">
            ...
            <div class="mb-6 max-w-md mx-auto">
                ...
                <div class="relative">
                    <input type="text" 
                        id="serial_number" 
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none text-lg text-center transition duration-200"
                ...
                <p class="text-xs text-gray-500 mt-2 text-center">
                    ...
                    Final step - PCB will be marked as COMPLETED
                </p>
            </div>
            ...
            <!-- Completion Message -->
            <div id="completionMessage" class="hidden mb-4 p-6 bg-green-50 border border-green-200 rounded-lg text-center">
                <i class="fas fa-check-circle text-green-500 text-4xl mb-3"></i>
                <h4 class="text-xl font-bold text-green-700 mb-2">🎉 PCB PROCESS COMPLETE!</h4>
                <p class="text-sm text-green-600 mb-3">All inspections passed. PCB ready for shipping.</p>
                <div class="flex justify-center space-x-2 text-xs text-gray-500">
                    <span class="bg-white px-2 py-1 rounded">FCT ✓</span>
                    <span class="bg-white px-2 py-1 rounded">LED ✓</span>
                    <span class="bg-white px-2 py-1 rounded">Visual ✓</span>
                </div>
            </div>

            <!-- Recent Completed PCBs -->
            <div>
...
$(document).ready(function() {
    loadRecentScans(1);
    checkSystemLock();
    lockCheckInterval = setInterval(checkSystemLock, 3000);
...
function loadRecentScans(page) {
    $.ajax({
        url: '/api/recent-scans/visual_inspection',
        method: 'GET',
        ...
        success: function(response) {
            ...
            if (data.length > 0) {
                    ...
                    html += `
                        <div class="flex items-center justify-between mt-3 pt-2 border-t border-gray-200">
                            <button onclick="changePage('prev')" 
                                ...
                                class="text-xs px-3 py-1 border rounded ${currentPage === 1 ? 'text-gray-300 border-gray-200' : 'text-gray-600 border-gray-300 hover:bg-gray-100'}">
                                ← Prev
                            </button>
                            <span class="text-xs text-gray-500">Page ${currentPage} of ${totalPages}</span>
                            <button onclick="changePage('next')" 
                                ${currentPage === totalPages ? 'disabled' : ''} 
                                class="text-xs px-3 py-1 border rounded ${currentPage === totalPages ? 'text-gray-300 border-gray-200' : 'text-gray-600 border-gray-300 hover:bg-gray-100'}">
                                Next →
                            </button>
                        </div>
                    `;
                ...
                $('#recent-scans').html('<p class="text-center py-4 text-gray-400 text-sm">No completions today</p>');
                $('#refreshTime').text('');
            }
        },
        error: function() {
...
function processScan() {
    ...
    $.ajax({
        url: '{{ route("pcb-scan.process", "visual_inspection") }}',
        method: 'POST',
        data: {
...
function showResult(type, message) {
    ...
    resultDiv.removeClass('hidden').show();
    ...
    resultDiv.html(`
        <div class="${s.bg} border ${s.border} ${s.text} px-4 py-4 rounded-lg flex items-center justify-between shadow-sm" role="alert">
            <div class="flex items-center">
                ...
                <div>
                    <span class="font-medium">${type === 'success' ? '✅ Success!' : type === 'error' ? '❌ Error!' : type === 'warning' ? '⚠️ Warning!' : 'ℹ️ Info!'}</span>
