<div class="p-1 space-y-2">
    ...
    <flux:card class="p-6">
        ...
        @php
            ...
            // HAPUS: $allCostingFeedbackFilled = true;
            ...
            // HAPUS: $costingReady = ($allVerificationFilled && $hasApprovedItems) || $hasManualItems;
            // HAPUS: $allCostingCompleted = true;
            // HAPUS: foreach untuk cek costing completed
        @endphp
            ...
            <table class="w-full text-sm" style="min-width: 1800px;">
                <thead class="bg-zinc-100 dark:bg-zinc-800">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-center text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider whitespace-nowrap">TANGGAL PEMBERKASAN</th>
                        <th class="px-3 py-2 text-center text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider whitespace-nowrap">DOJ</th>
                        ...
                        <th class="px-3 py-2 text-center text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider whitespace-nowrap">REASON TYPE</th>
                        <th class="px-3 py-2 text-center text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider whitespace-nowrap">REASON FILE</th>
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                    @php
                        $groupedItems = $this->grouped_items;
                        $counter = 0;
                    @endphp
                    @foreach($groupedItems as $group)
                        ...
                        @foreach($group['items'] as $itemData)
                            @php
                                ...
                                $hasAdminFeedback = !empty($item['admin_feedback']) && $item['admin_feedback'] !== 'N/A (Manual Input)';
                                $misscStatus = $request->missc_status ?? 'Waiting';
                                $canSign = $isApproved && $misscStatus === 'On Process';
                            @endphp
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                <!-- # - Merge -->
                                @if($isFirst)
                                    <td class="px-3 py-2 text-center text-sm whitespace-nowrap align-middle" rowspan="{{ $rowspan }}">
                                    ...
                                    <!-- NAME - Merge -->
                                    <td class="px-3 py-2 text-left text-sm whitespace-nowrap align-middle" rowspan="{{ $rowspan }}">
                                        {{ $group['employee_name'] }}
                                    </td>
                                    <!-- DEPARTMENT - Merge -->
                                    <td class="px-3 py-2 text-left text-sm whitespace-nowrap align-middle" rowspan="{{ $rowspan }}">
                                    ...
                                    <td class="px-3 py-2 text-center text-sm whitespace-nowrap align-middle" rowspan="{{ $rowspan }}">
                                        @if(isset($group['manual_tanggal_pemberkasan']) && $group['manual_tanggal_pemberkasan'])
                                            {{ \Carbon\Carbon::parse($group['manual_tanggal_pemberkasan'])->format('d/m/Y') }}
                                        @else
                                    ...
                                    <td class="px-3 py-2 text-center text-sm whitespace-nowrap align-middle" rowspan="{{ $rowspan }}">
                                        @if(isset($group['manual_doj']) && $group['manual_doj'])
                                            {{ \Carbon\Carbon::parse($group['manual_doj'])->format('d/m/Y') }}
                                        @else
                                            <span class="text-xs text-zinc-400">-</span>
                                        @endif
                                    </td>
                                ...
                                <td class="px-3 py-2 text-center text-sm whitespace-nowrap">
                                    ...
                                    @elseif($item['status'] == 'System')
                                        <flux:badge size="xs" color="green">System</flux:badge>
                                    @elseif($item['status'] == 'Not Use')
                                        <flux:badge size="xs" color="gray">Not Use</flux:badge>
                                    @else
                                        <flux:badge size="xs" color="yellow">{{ $item['status'] }}</flux:badge>
                                    @endif
                                </td>
                                ...
                                <td class="px-3 py-2 text-center text-sm whitespace-nowrap">
                                    ...
                                    @if($reasonType === 'ng_esd')
                                        @if(!empty($item['reason']))
                                            {{ $item['reason'] }}
                                        @else
                                            <span class="text-xs text-red-600 dark:text-red-400 font-medium">NG ESD</span>
                                        @endif
                                    @else
                                        {{ $item['reason'] ?? '-' }}
                                    @endif
                                ...
                                <!-- REASON TYPE -->
                                <td class="px-3 py-2 text-center text-sm whitespace-nowrap">
                                    @if(isset($item['reason_type']))
                                        @if($item['reason_type'] === 'ng_esd')
                                        ...
                                        @elseif($item['reason_type'] === 'new_employee')
                                    ...
                                    <td class="px-3 py-2 text-center text-sm whitespace-nowrap">
                                        ...
                                        @else
                                            @can('feedback uniform request admin')
                                                ...
                                                @if($isClosed)
                                                    <button type="button" 
                                                        wire:click="openSalaryModal({{ $index }})" 
                                    ...
                                    @else
                                        @can('verify uniform request')
                                            @if(isset($item['admin_feedback']) && $item['admin_feedback'])
                                                <button type="button" 
                                                    wire:click="openVerificationModal({{ $index }})" 
                                ...
                                <td class="px-3 py-2 text-center text-sm whitespace-nowrap">
                                    @php
                                        ...
                                        $isManual = isset($item['is_manual']) && $item['is_manual'];
                                        $isApproved = isset($item['verification_status']) && $item['verification_status'] === 'approved';
                                        $hasAdminFeedback = !empty($item['admin_feedback']) && $item['admin_feedback'] !== 'N/A (Manual Input)' && $item['admin_feedback'] !== 'N/A (New Employee)';
                                        $misscStatus = $request->missc_status ?? 'Waiting';
                                        
                                        // New Employee: bisa sign langsung tanpa perlu approved
                                        if ($isNewEmployee) {
                                            $canSign = true;
                                        } else {
                                            $canSign = $isApproved && $misscStatus === 'On Process';
                                            ...
                                            @else
                                                ...
                                                @elseif($canSign)
                                                    @can('sign uniform request')
                                                        <button type="button" 
                                                            wire:click="openSignatureModal({{ $index }})" 
                                                            wire:key="sign-btn-{{ $index }}-{{ time() }}"
                                                            class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-indigo-700 bg-indigo-100 rounded-lg hover:bg-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-300 dark:hover:bg-indigo-900/50 transition-colors whitespace-nowrap {{ $isNewEmployee ? 'border-2 border-indigo-500 animate-pulse' : '' }}">
                                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-4">
                                                    ...
                                                    @endcan
                                                @elseif($isApproved && $misscStatus !== 'On Process' && !$isNewEmployee)
                                                ...
                                                @elseif($hasAdminFeedback && empty($item['verification_status']) && !$isNewEmployee)
                                                    <span class="text-xs text-yellow-600 dark:text-yellow-400 whitespace-nowrap flex items-center gap-1 justify-center">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                                        </svg>
                                                ...
                                                @else
                                                    <span class="text-xs text-zinc-400 italic whitespace-nowrap">Waiting from Admin</span>
                                                @endif
                                            @endif
                                            ...
                                            <div class="fixed z-[9999] pointer-events-none"
                                                ...
                                                <div class="bg-zinc-900 text-white text-xs rounded-lg p-3 shadow-2xl min-w-[220px] max-w-[300px]">
                                                    <div class="space-y-1 text-left">
                                                        ...
                                                        @if($isNewEmployee)
                                                            <p><span class="text-zinc-400">Type:</span> <span class="text-green-400">New Employee</span></p>
                                                        @endif
        ...
        initCanvas() {
            ...
            function stopDraw(e) {
                ...
                if (isDrawing) {
                    ...
                    // Gunakan wire:model langsung
                    $wire.signatureImage = imageData;
                }
            }
            ...
            // Gunakan canvas baru
            const finalCanvas = newCanvas;
            const finalCtx = newCanvas.getContext('2d');
            let finalIsDrawing = false;
            let finalLastX = 0;
            let finalLastY = 0;

            // Background putih untuk canvas baru
            finalCtx.fillStyle = '#ffffff';
            finalCtx.fillRect(0, 0, finalCanvas.width, finalCanvas.height);
            
            // Load signature yang sudah ada ke canvas baru
            if ($wire.signatureImage) {
                const img = new Image();
                img.onload = function() {
            ...
            function finalStartDraw(e) {
                e.preventDefault();
                finalIsDrawing = true;
                const pos = finalGetPos(e);
                finalLastX = pos.x;
                finalLastY = pos.y;
            }

            function finalDraw(e) {
                e.preventDefault();
                if (!finalIsDrawing) return;
                const pos = finalGetPos(e);
                finalCtx.beginPath();
                finalCtx.moveTo(finalLastX, finalLastY);
            ...
            function finalStopDraw(e) {
                e.preventDefault();
                if (finalIsDrawing) {
                    finalIsDrawing = false;
            ...
            // Event listeners
            finalCanvas.addEventListener('mousedown', finalStartDraw);
            finalCanvas.addEventListener('mousemove', finalDraw);
            finalCanvas.addEventListener('mouseup', finalStopDraw);
            finalCanvas.addEventListener('mouseleave', finalStopDraw);

            finalCanvas.addEventListener('touchstart', finalStartDraw, { passive: false });
            finalCanvas.addEventListener('touchmove', finalDraw, { passive: false });
            finalCanvas.addEventListener('touchend', finalStopDraw, { passive: false });
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
                ...
                <div class="p-6 overflow-y-auto flex-1">
                    ...
                    <div class="mb-4 overflow-x-auto">
                        <table class="w-full text-sm border rounded-lg">
                            <tbody>
                                ...
                                <tr class="border-b">
                                    <td class="px-3 py-2 font-medium text-zinc-600 dark:text-zinc-400">Department</td>
                                    <td class="px-3 py-2">: {{ $signatureItem['employee_department'] ?? '-' }}</td>
                                </tr>
                                <tr class="border-b">
                                    <td class="px-3 py-2 font-medium text-zinc-600 dark:text-zinc-400">Item</td>
                                    <td class="px-3 py-2">: {{ $signatureItem['item_code'] ?? '-' }} - {{ $signatureItem['description'] ?? '' }}</td>
                                </tr>
                                <tr>
                                    <td class="px-3 py-2 font-medium text-zinc-600 dark:text-zinc-400">Qty</td>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="border-2 border-zinc-300 dark:border-zinc-600 rounded-lg overflow-hidden bg-white">
                            <canvas id="signatureCanvas" 
                                class="w-full h-64 touch-none cursor-crosshair"
                                style="touch-action: none; display: block; background: white; width: 100%; height: 256px;"
                            ></canvas>
                        </div>
                ...
                <!-- Footer -->
                <div class="p-6 border-t border-zinc-200 dark:border-zinc-700 flex justify-end gap-2 flex-shrink-0">
                    <button @click="closeModal()" 
                        class="px-4 py-2 border rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors">
                        Cancel
                    </button>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="mt-4 p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg">
                                ...
                                <p class="text-[10px] font-mono text-zinc-500 break-all">
                                    NIK,NAME,DEPARTMENT,ITEM CODE,DESCRIPTION,SIZE,QTY,GROUP,REQUEST DATE,REASON,REASON TYPE,REMARKS,ADMIN FEEDBACK,SALARY DEDUCTION
                                </p>
                                <p class="text-[10px] text-zinc-400 mt-1">
                                    Note: 
                                    - REASON is only required for "Pergantian" type (optional for "NG ESD")
                                    - REASON TYPE must be "Pergantian" or "NG ESD"
                                    - SALARY DEDUCTION (Yes/No) is optional
                                </p>
                            </div>
                        </div>
                    @else
                                        ...
                                        <table class="w-full text-sm" style="min-width: max-content; white-space: nowrap;">
                                            <thead class="bg-red-50 dark:bg-red-900/20">
                                                <tr>
                                                    ...
                                                    <th class="px-3 py-2 text-center">SALARY DEDUCTION</th>
                                            ...
                                            <tbody>
                                                @foreach($importErrors as $error)
                                                    <tr class="border-t border-red-100 dark:border-red-900/50 hover:bg-red-50 dark:hover:bg-red-900/10">
                                                        ...
                                                        <td class="px-3 py-2">{{ $error['data']['REASON TYPE'] ?? '-' }}</td>
                                                        <td class="px-3 py-2">{{ $error['data']['REMARKS'] ?? '-' }}</td>
                                                        <td class="px-3 py-2 max-w-xs">
                                                            <div class="text-xs break-words">{{ $error['data']['ADMIN FEEDBACK'] ?? '-' }}</div>
                                                        </td>
                                                        <td class="px-3 py-2 text-center">{{ $error['data']['SALARY DEDUCTION'] ?? '-' }}</td>
                                                        <td class="px-3 py-2 text-center">{{ $error['data']['AMOUNT'] ?? '-' }}</td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
                                ...
                                <div class="mb-4">
                                        ...
                                        <table class="w-full text-sm border-collapse" style="min-width: max-content;">
                                            <thead class="bg-green-50 dark:bg-green-900/20">
                                                <tr>
                                                    ...
                                                    <th class="px-3 py-2 text-left border-b border-green-200 dark:border-green-800">REASON TYPE</th>
                                                    <th class="px-3 py-2 text-left border-b border-green-200 dark:border-green-800">REMARKS</th>
                                                    <th class="px-3 py-2 text-left border-b border-green-200 dark:border-green-800">ADMIN FEEDBACK</th>
                                            ...
                                            <tbody>
                                                @foreach($importPreview as $preview)
                                                    <tr class="border-b border-green-100 dark:border-green-900/50 hover:bg-green-50 dark:hover:bg-green-900/10">
                                                        ...
                                                        <td class="px-3 py-2">
                                                            @if(isset($preview['reason_type']) && $preview['reason_type'] === 'ng_esd')
                                                                <span class="text-xs text-red-600 dark:text-red-400 font-medium">NG ESD</span>
                                                            @else
                                                                <span class="text-xs text-blue-600 dark:text-blue-400 font-medium">Pergantian</span>
                                                            @endif
                                                        </td>
                                    ...
                                    <div class="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                                        <p class="text-xs text-blue-700 dark:text-blue-400 flex items-start gap-2">
                                            ...
                                            <span><strong>Note:</strong> Salary Deduction will be auto-calculated from Master Uniform price.</span>
                                        </p>
                                    </div>
                                </div>
                            @endif
