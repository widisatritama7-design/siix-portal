    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-2">
        <div>
            ...
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                Filter and export Blind Test statistic per Defect item
            </p>
        </div>
    </div>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="bg-zinc-50 dark:bg-zinc-800/50">
                                ...
                                <th class="px-4 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Defect Name</th>
                                <th class="px-4 py-3 text-center text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider w-32">FAIL</th>
                                <th class="px-4 py-3 text-center text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider w-32">PASS</th>
                                <th class="px-4 py-3 text-center text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider w-32">TOTAL</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                            ...
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                ...
                                <td class="px-4 py-3 text-center">
                                    @if($item['fail'] > 0)
                                        <flux:badge size="sm" color="red">{{ $item['fail'] }}</flux:badge>
                                    @else
                                ...
                                <td class="px-4 py-3 text-center">
                                    @if($item['pass'] > 0)
                                        <flux:badge size="sm" color="green">{{ $item['pass'] }}</flux:badge>
                                    @else
                                        <span class="text-sm text-zinc-400">0</span>
                                    @endif
                                </td>
                        ...
                        <tfoot>
                            <tr class="bg-amber-50 dark:bg-amber-950/20 border-t-2 border-amber-300 dark:border-amber-700">
                                <td colspan="2" class="px-4 py-3 text-right text-sm font-bold text-zinc-800 dark:text-white uppercase">
                                    Total
                                </td>
