{{-- resources/views/livewire/esd/magazine/magazine-management.blade.php --}}
<section class="w-full">
    @include('partials.esd-heading')

    <flux:heading class="sr-only">
        {{ __('Electrostatic Discharge - Magazine') }}
    </flux:heading>
        ...
        <div class="-mt-2">
            ...
            <div class="flex flex-col sm:flex-row gap-3 mt-2 mb-6">
                ...
                <div class="w-full sm:w-[20%]">
                    <flux:button 
                        href="{{ route('esd.magazine-details')}}"
                        wire:navigate
                        icon="arrow-right"
                        variant="primary"
                        color="green"
                ...
                <div class="w-full sm:w-[20%]">
                    ...
                    <flux:button 
                        ...
                        x-on:click="$dispatch('open-modal', 'magazine-form-modal')"
                    >
                        Add New
                    </flux:button>
                    @endcan
                ...
                <div class="overflow-x-auto">
                    <table class="w-full whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                            ...
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors" wire:key="magazine-{{ $magazine->id }}">
                                <td class="px-4 py-3 text-sm text-zinc-500 dark:text-zinc-400">
                                    {{ $magazines->firstItem() + $index }}
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center text-white font-medium shadow-lg flex-shrink-0">
                                            {{ strtoupper(substr($magazine->register_no, 0, 1)) }}
                                        </div>
                                        <div class="min-w-0">
                                            <span class="text-sm font-semibold text-zinc-800 dark:text-white block truncate max-w-[200px]" title="{{ $magazine->register_no }}">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm font-semibold text-zinc-800 dark:text-white">
                                            {{ $magazine->magazineDetails()->count() }}
                                        </span>
                                ...
                                <td class="px-4 py-3">
                                    <div class="text-sm min-w-0">
                                        <div class="truncate max-w-[150px]" title="{{ $magazine->creator->name ?? 'N/A' }}">
                                            {{ $magazine->creator->name ?? 'N/A' }}
                                        </div>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1 whitespace-nowrap">
                                        @can('view magazine')
                                        <flux:button 
                                            href="{{ route('esd.magazines.show', $magazine->id) }}"
                                            wire:navigate
                                            size="sm"
                                            icon="eye"
                                            variant="primary"
                                        ...
                                        @can('delete magazine')
                                            <flux:button 
                                                wire:click="confirmDelete({{ $magazine->id }})" 
                                                x-on:click="$dispatch('open-modal', 'delete-magazine-modal')"
                                                size="sm"
                                                icon="trash"
                                                variant="primary"
                                                color="red"
                                    ...
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
                                                {{ $search ? 'Try adjusting your search query' : 'Get started by creating a new magazine record' }}
                                            </p>
                                        </div>
                                        @if($search)
                ...
                <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-xl w-full max-w-md p-6 text-center">
                        ...
                        <p class="text-gray-600 dark:text-gray-400 mb-6">
                            Are you sure you want to delete magazine "{{ $magazineToDelete?->register_no }}"? This action cannot be undone.
                        </p>
