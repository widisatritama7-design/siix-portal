{{-- resources/views/livewire/esd/magazine/magazine-detail-management.blade.php --}}
<section class="w-full">
    @include('partials.esd-heading')

    <flux:heading class="sr-only">
        {{ __('Electrostatic Discharge - Magazine Measurement') }}
    </flux:heading>
            ...
            <div x-data="{ showFilters: false }">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-1 mb-4">
                    ...
                    <div class="flex gap-2">
                        @if($filterMagazine || $filterJudgementM1 || $filterJudgementM2 || $filterDateFrom || $filterDateUntil || $filterNextDateFrom || $filterNextDateUntil || $search)
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center text-white font-medium shadow-lg flex-shrink-0 text-xs">
                                            {{ strtoupper(substr($detail->magazine->register_no ?? '', 0, 1)) }}
                                        </div>
                                        <span class="text-sm font-semibold text-zinc-800 dark:text-white truncate max-w-[180px]" title="{{ $detail->magazine->register_no ?? '' }}">
                                            {{ $detail->magazine->register_no ?? 'N/A' }}
                                        </span>
                                    </div>
                                </td>
                                    ...
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
                                                {{ $search || $filterMagazine ? 'Try adjusting your filters' : 'Get started by adding a new measurement' }}
                                            </p>
                                        </div>
                                        @if($search || $filterMagazine)
                                ...
                                <div class="mb-4">
                                    ...
                                    <select wire:model="magazine_id"
                                        ...
                                        @foreach($magazines as $magazine)
                                            <option value="{{ $magazine->id }}">{{ $magazine->register_no }}</option>
                                        @endforeach
                                    </select>
                                    @error('magazine_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
