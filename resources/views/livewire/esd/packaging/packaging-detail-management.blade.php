<section class="w-full">
    ...
    <flux:heading class="sr-only">
        {{ __('Electrostatic Discharge - Packaging Measurement') }}
    </flux:heading>
        ...
        <div class="-mt-2">
            ...
            <div x-data="{ showFilters: false }">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-1 mb-4">
                    ...
                    <div class="flex gap-2">
                        @if($filterPackaging || $filterJudgementF1 || $filterJudgementF2 || $filterDateFrom || $filterDateUntil || $filterNextDateFrom || $filterNextDateUntil || $search)
            ...
            <div class="mb-4">
                <div class="bg-white dark:bg-zinc-800 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700 shadow-sm">
                    ...
                    @if($printMaterialFilter || $printCategoryFilter || $printDateFrom || $printDateUntil)
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center text-white font-medium shadow-lg flex-shrink-0 text-xs">
                                            {{ strtoupper(substr($detail->packaging->material ?? '', 0, 1)) }}
                                        </div>
                                        <div class="min-w-0">
                                            <span class="text-sm font-semibold text-zinc-800 dark:text-white block truncate max-w-[180px]" title="{{ $detail->packaging->material ?? '' }}">
                                                {{ $detail->packaging->material ?? 'N/A' }}
                                            </span>
                                            @if($detail->packaging->category)
                                            <div class="text-xs text-zinc-500 dark:text-zinc-400">
                                                {{ $detail->packaging->category }} | {{ $detail->packaging->project ?? 'No Project' }}
                                            </div>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
                                                {{ $search || $filterPackaging ? 'Try adjusting your filters' : 'Get started by adding a new measurement' }}
                                            </p>
                                        </div>
                                        @if($search || $filterPackaging)
                        ...
                        <div class="p-6">
                            ...
                            <form wire:submit="save">
                                ...
                                <div class="mb-4">
                                    ...
                                    @error('packaging_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
