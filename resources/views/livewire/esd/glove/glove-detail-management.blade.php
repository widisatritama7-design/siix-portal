{{-- resources/views/livewire/esd/glove/glove-detail-management.blade.php --}}
<section class="w-full">
    @include('partials.esd-heading')

    <flux:heading class="sr-only">
        {{ __('Electrostatic Discharge - Glove Measurement') }}
    </flux:heading>
        ...
        <div class="-mt-2">
            ...
            <div x-data="{ showFilters: false }">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-1 mb-4">
                    ...
                    <div class="flex gap-2">
                        @if($filterGlove || $filterSapCode || $filterDescription || $filterDelivery || $filterJudgement || $filterDateFrom || $filterDateUntil || $filterNextDateFrom || $filterNextDateUntil || $search)
            ...
            <div class="mb-4">
                <div class="bg-white dark:bg-zinc-800 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700 shadow-sm">
                    ...
                    @if($printSapCode || $printDescription || $printDelivery || $printDateFrom || $printDateUntil)
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-medium shadow-lg flex-shrink-0 text-xs">
                                            {{ strtoupper(substr($detail->glove->sap_code ?? '', 0, 1)) }}
                                        </div>
                                        <span class="text-sm font-semibold text-zinc-800 dark:text-white truncate max-w-[180px]" title="{{ $detail->glove->sap_code ?? '' }}">
                                            {{ $detail->glove->sap_code ?? 'N/A' }}
                                        </span>
                                    </div>
                                 </>
                                <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300 truncate max-w-[200px]" title="{{ $detail->glove->description ?? '' }}">
                                    {{ $detail->glove->description ?? 'N/A' }}
                                 </>
                                <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300">
                                    {{ $detail->glove->delivery ?? '-' }}
                                 </>
                                    ...
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
                                                {{ $search || $filterGlove || $filterSapCode || $filterDescription || $filterDelivery ? 'Try adjusting your filters' : 'Get started by adding a new measurement' }}
                                            </p>
                                        </div>
                                        @if($search || $filterGlove || $filterSapCode || $filterDescription || $filterDelivery)
                                ...
                                <div class="mb-4">
                                    ...
                                    <select wire:model="glove_id"
                                        ...
                                        @foreach($gloves as $glove)
                                            <option value="{{ $glove->id }}">{{ $glove->sap_code }} - {{ $glove->description }} - {{ $glove->delivery }}</option>
                                        @endforeach
                                    </select>
                                    @error('glove_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
