...
<body>
    <div class="container">
                    ...
                    <td style="width: 50%; vertical-align: top;">
                        <table style="width: 100%;">
                            ...
                            <tr>
                                ...
                                <td colspan="3">: {{ $judgement_filter }}</td>
                            </tr>
                            @endif
        ...
        <table class="data-table">
            ...
            <tbody>
                @forelse($details as $index => $patrol)
                <tr>
                    <td class="text-center">{{ $index + 1 }}</td>
                    <td class="text-center">{{ $patrol->area ?? 'N/A' }}</td>
                    <td class="text-center">{{ $patrol->location ?? '-' }}</td>
                    <td class="text-center">
                        <span class="{{ $patrol->v_1 == 'Connected' ? 'badge-connected' : 'badge-not-connected' }}">
                            {{ $patrol->v_1 ?? '-' }}
                        </span>
                    </td>
                    <td class="text-center">
                        <span class="{{ $patrol->v_2 == 'Good' ? 'badge-good' : 'badge-not-good' }}">
                            {{ $patrol->v_2 ?? '-' }}
                        </span>
                    </td>
                    <td class="text-center">
                    ...
                    <td class="text-center">
                        <span class="{{ $patrol->judgement_v3 == 'Good' ? 'badge-good' : 'badge-not-good' }}">
                            {{ $patrol->judgement_v3 ?? '-' }}
                        </span>
                    </td>
                    <td class="text-center">
                        <span class="{{ $patrol->v_4 == 'Good' ? 'badge-good' : 'badge-not-good' }}">
                            {{ $patrol->v_4 ?? '-' }}
                        </span>
                    </td>
                    <td class="wrap-text">{{ $patrol->remarks ?? '-' }}</td>
                    <td class="wrap-text">{{ $patrol->creator->name ?? 'N/A' }}</td>
                    <td class="text-center">{{ $patrol->created_at ? $patrol->created_at->format('d M Y') : '-' }}</td>
                    <td class="text-center">{{ $patrol->next_date ? \Carbon\Carbon::parse($patrol->next_date)->format('d M Y') : '-' }}</td>
                </tr>
                @empty
                <tr>
