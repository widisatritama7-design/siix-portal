...
<body>
    <div class="container">
        ...
        <table class="data-table">
            ...
            <tbody>
                @forelse($details as $index => $check)
                <tr>
                    <td class="text-center">{{ $index + 1 }}</td>
                    <td class="text-center">{{ $check->register_no ?? 'N/A' }}</td>
                    <td class="text-center">{{ $check->result_scientific ?? '-' }}</td>
                    <td class="text-center">
                        <span class="{{ $check->judgement == 'OK' ? 'badge-ok' : 'badge-ng' }}">
                            {{ $check->judgement ?? '-' }}
                        </span>
                    </td>
                    <td class="wrap-text">{{ $check->remarks ?? '-' }}</td>
                    <td class="text-center">{{ $check->created_at ? $check->created_at->format('d M Y') : '-' }}</td>
                    <td class="text-center">{{ $check->next_date ? \Carbon\Carbon::parse($check->next_date)->format('d M Y') : '-' }}</td>
                    <td class="wrap-text">{{ $check->creator->name ?? 'N/A' }}</td>
                </tr>
                @empty
                <tr>
