{{-- resources/views/livewire/esd/jig/jig-detail-management.blade.php --}}
<section class="w-full">
    @include('partials.esd-heading')

    <flux:heading class="sr-only">
        {{ __('Electrostatic Discharge - Jig Measurement') }}
    </flux:heading>
        ...
        <div class="-mt-2">
            ...
            <div x-data="{ showFilters: false }">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-1 mb-4">
                    ...
                    <div class="flex gap-2">
                        @if($filterJig || $filterLocation || $filterJudgementJ1 || $filterJudgementJ2 || $filterDateFrom || $filterDateUntil || $filterNextDateFrom || $filterNextDateUntil || $search)
            ...
            <div class="mb-4">
                <div class="bg-white dark:bg-zinc-800 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700 shadow-sm">
                    ...
                    @if($printRegisterNo || $printLocation || $printDateFrom || $printDateUntil)
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center text-white font-medium shadow-lg flex-shrink-0 text-xs">
                                            {{ strtoupper(substr($detail->jig->register_no ?? '', 0, 1)) }}
                                        </div>
                                        <span class="text-sm font-semibold text-zinc-800 dark:text-white truncate max-w-[180px]" title="{{ $detail->jig->register_no ?? '' }}">
                                            {{ $detail->jig->register_no ?? 'N/A' }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300">
                                    {{ $detail->jig->location ?? 'N/A' }}
                                </td>
                                    ...
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
                                                {{ $search || $filterJig || $filterLocation ? 'Try adjusting your filters' : 'Get started by adding a new measurement' }}
                                            </p>
                                        </div>
                                        @if($search || $filterJig || $filterLocation)
                            ...
                            <form wire:submit="save">
                                ...
                                <div class="mb-4">
                                    ...
                                    <select wire:model="jigs_id"
                                        ...
                                        @foreach($jigs as $jig)
                                            <option value="{{ $jig->id }}">{{ $jig->register_no }} - {{ $jig->location }}</option>
                                        @endforeach
                                    </select>
                                    @error('jigs_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
