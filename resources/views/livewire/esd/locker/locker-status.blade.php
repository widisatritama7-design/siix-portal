...
<body>
            ...
            <div class="column">
                ...
                <div class="locker-list">
                    @for ($i = 1; $i <= 10; $i++)
                        ...
                        @endphp
                        <div class="locker-item {{ $locker ? '' : 'no-data' }}" 
                             data-locker-number="{{ $i }}"
                             data-nik="{{ $locker->nik ?? '' }}"
                             data-name="{{ $locker->name ?? '' }}"
                             data-dept="{{ $locker->dept ?? '' }}"
                             data-status="{{ $locker->status ?? 'Available' }}">
                            <div class="locker-number">
                                <i class="fas {{ $locker ? getStatusIcon($locker->status) : 'fa-unlock' }}"></i>
                                Locker {{ $i }}
                            </div>
                            @if ($locker)
                                <div class="locker-info">
                                    <div class="locker-meta nik">{{ $locker->nik ?? '-' }}</div>
                                    <div class="locker-meta name">{{ $locker->name ?? '-' }}</div>
                                    <div class="locker-meta dept">{{ $locker->dept ?? '-' }}</div>
                                </div>
                                <div class="locker-meta status">
                                    <span class="status-badge status-{{ str_replace(' ', '-', $locker->status) }}">
                                        <i class="fas {{ getStatusIcon($locker->status) }}"></i>
                                        {{ $locker->status }}
                                    </span>
                                    @if($locker->status === 'Finish')
                                        <button class="btn-check" data-locker-number="{{ $i }}">
    ...
    <script>
        ...
        async function resetLocker(lockerNumber) {
            try {
                const response = await fetch("{{ route('locker.update') }}", {
                    method: 'POST',
                    headers: {
        ...
        async function fetchLockerData() {
            try {
                const response = await fetch("{{ route('locker.status') }}");
                const data = await response.json();
