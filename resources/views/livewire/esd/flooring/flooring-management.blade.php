{{-- resources/views/livewire/esd/flooring/flooring-management.blade.php --}}
<section class="w-full">
    @include('partials.esd-heading')

    <flux:heading class="sr-only">
        {{ __('Electrostatic Discharge - Flooring') }}
    </flux:heading>
        ...
        <div class="-mt-2">
            ...
            <div class="flex flex-col sm:flex-row gap-3 mt-2 mb-6">
                ...
                <div class="w-full sm:w-[20%]">
                    <flux:button 
                        href="{{ route('esd.flooring-details')}}"
                        wire:navigate
                        icon="arrow-right"
                        variant="primary"
                        color="green"
                ...
                <div class="w-full sm:w-[20%]">
                    ...
                    <flux:button 
                        ...
                        x-on:click="$dispatch('open-modal', 'flooring-form-modal')"
                    >
                        Add New
                    </flux:button>
                    @endcan
                ...
                <div class="overflow-x-auto">
                    <table class="w-full whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                            @forelse($floorings as $index => $flooring)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors" wire:key="flooring-{{ $flooring->id }}">
                                <td class="px-4 py-3 text-sm text-zinc-500 dark:text-zinc-400">
                                    {{ $floorings->firstItem() + $index }}
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center text-white font-medium shadow-lg flex-shrink-0">
                                            {{ strtoupper(substr($flooring->register_no, 0, 1)) }}
                                        </div>
                                        <div class="min-w-0">
                                            <span class="text-sm font-semibold text-zinc-800 dark:text-white block truncate max-w-[200px]" title="{{ $flooring->register_no }}">
                                                {{ $flooring->register_no }}
                                            </span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="text-sm text-zinc-700 dark:text-zinc-300 block truncate max-w-[150px]" title="{{ $flooring->area }}">
                                        {{ $flooring->area }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="text-sm text-zinc-700 dark:text-zinc-300 block truncate max-w-[150px]" title="{{ $flooring->location }}">
                                        {{ $flooring->location }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm font-semibold text-zinc-800 dark:text-white">
                                            {{ $flooring->flooringDetails()->count() }}
                                        </span>
                                ...
                                <td class="px-4 py-3">
                                    <div class="text-sm min-w-0">
                                        <div class="truncate max-w-[150px]" title="{{ $flooring->creator->name ?? 'N/A' }}">
                                            {{ $flooring->creator->name ?? 'N/A' }}
                                        </div>
                                        <div class="text-xs text-zinc-500 whitespace-nowrap">
                                            {{ $flooring->created_at->format('d M Y') }}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1 whitespace-nowrap">
                                        @can('view flooring')
                                        <flux:button 
                                            href="{{ route('esd.floorings.show', $flooring->id) }}"
                                            wire:navigate
                                            size="sm"
                                            icon="eye"
                                            variant="primary"
                                        ...
                                        @can('delete flooring')
                                            <flux:button 
                                                wire:click="confirmDelete({{ $flooring->id }})" 
                                                x-on:click="$dispatch('open-modal', 'delete-flooring-modal')"
                                                size="sm"
                                                icon="trash"
                                                variant="primary"
                                                color="red"
                                                class="!p-2 flex-shrink-0"
                                                title="Delete flooring"
                                            />
                                        @endcan
                                    </div>
                                </td>
                                    ...
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
                                                {{ $search ? 'Try adjusting your search query' : 'Get started by creating a new flooring record' }}
                                            </p>
                                        </div>
                                        @if($search)
                ...
                <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-xl w-full max-w-md p-6 text-center">
                        ...
                        <p class="text-gray-600 dark:text-gray-400 mb-6">
                            Are you sure you want to delete flooring "{{ $flooringToDelete?->register_no }}"? This action cannot be undone.
                        </p>
