<section class="w-full">
    ...
    <flux:heading class="sr-only">
        {{ __('Electrostatic Discharge - Soldering Measurement') }}
    </flux:heading>
        ...
        <div class="-mt-2">
            ...
            <div class="mb-4">
                <div class="bg-white dark:bg-zinc-800 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700 shadow-sm">
                    ...
                    @if($printRegisterFilter || $printAreaFilter || $printLocationFilter || $printDateFrom || $printDateUntil)
            ...
            <div class="mb-4 text-right">
                @if($filterSoldering || $filterArea || $filterLocation || $filterType || $filterLine || $filterJudgement || $filterDateFrom || $filterDateUntil || $filterNextDateFrom || $filterNextDateUntil || $search)
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center text-white font-medium shadow-lg flex-shrink-0 text-xs">
                                            {{ strtoupper(substr($detail->soldering->register_no ?? '', 0, 1)) }}
                                        </div>
                                        <span class="text-sm font-semibold text-zinc-800 dark:text-white truncate max-w-[180px]" title="{{ $detail->soldering->register_no ?? '' }}">
                                            {{ $detail->soldering->register_no ?? 'N/A' }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300">
                                    {{ $detail->soldering->area ?? 'N/A' }}
                                </td>
                                <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300">
                                    {{ $detail->soldering->location ?? 'N/A' }}
                                </td>
                                <td class="px-4 py-3">
                                    <span class="text-sm text-zinc-700 dark:text-zinc-300 uppercase">
                                        {{ $detail->soldering->type ?? '-' }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="text-sm text-zinc-700 dark:text-zinc-300 uppercase">
                                        {{ $detail->soldering->spec ?? '-' }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300">
                                    {{ $detail->soldering->line ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300">
                                    {{ $detail->e1 }}
                                </td>
                                    ...
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
                                                {{ $search || $filterSoldering || $filterArea || $filterLocation || $filterType || $filterLine ? 'Try adjusting your filters' : 'Get started by adding a new measurement' }}
                                            </p>
                                        </div>
                                        @if($search || $filterSoldering || $filterArea || $filterLocation || $filterType || $filterLine)
                            ...
                            <form wire:submit="save">
                                ...
                                <div class="mb-4">
                                    ...
                                    <select wire:model="soldering_id"
                                        ...
                                        @foreach($solderings as $soldering)
                                            <option value="{{ $soldering->id }}">{{ $soldering->register_no }} - {{ $soldering->area }} - {{ $soldering->location }}</option>
                                        @endforeach
                                    </select>
                                    @error('soldering_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    ...
                                    <div class="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
                                        ...
                                        <input type="number" step="0.01" wire:model="e1" wire:keyup="resetJudgement"
                                            class="w-full px-3 py-2 border rounded-lg dark:bg-zinc-800 dark:border-zinc-700 focus:ring-2 focus:ring-blue-500">
                                        @error('e1') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                    </div>
                ...
                <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-xl w-full max-w-md p-6 text-center">
                        ...
                        <p class="text-gray-600 dark:text-gray-400 mb-6">
                            Are you sure you want to delete measurement for "{{ $detailToDelete?->soldering?->register_no ?? 'this soldering' }}"? This action cannot be undone.
                        </p>
