<section class="w-full">
    ...
    <flux:heading class="sr-only">
        {{ __('Electrostatic Discharge - Soldering') }}
    </flux:heading>
    ...
    >
        ...
        <div class="-mt-2">
            ...
            <div class="flex flex-col sm:flex-row gap-3 mt-2 mb-6">
                ...
                <div class="w-full sm:flex-1">
                    <flux:input
                        ...
                        placeholder="Search by register number, area, location, type, spec, or line..."
                ...
                <div class="w-full sm:w-auto">
                    <flux:button 
                        href="{{ route('esd.soldering-details')}}"
                        wire:navigate
                        icon="arrow-right"
                        variant="primary"
                        color="green"
            ...
            <div class="mb-4 text-right">
                @if($filterArea || $filterLocation || $filterType || $filterLine || $search)
                ...
                <div class="overflow-x-auto">
                    <table class="w-full whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                            @forelse($solderings as $index => $soldering)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors" wire:key="soldering-{{ $soldering->id }}">
                                <td class="px-4 py-3 text-sm text-zinc-500 dark:text-zinc-400">
                                    {{ $solderings->firstItem() + $index }}
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center text-white font-medium shadow-lg flex-shrink-0">
                                            {{ strtoupper(substr($soldering->register_no, 0, 1)) }}
                                        </div>
                                        <div class="min-w-0">
                                            <span class="text-sm font-semibold text-zinc-800 dark:text-white block truncate max-w-[200px]" title="{{ $soldering->register_no }}">
                                                {{ $soldering->register_no }}
                                            </span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="text-sm text-zinc-700 dark:text-zinc-300 block truncate max-w-[150px]" title="{{ $soldering->area }}">
                                        {{ $soldering->area }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="text-sm text-zinc-700 dark:text-zinc-300 block truncate max-w-[150px]" title="{{ $soldering->location }}">
                                        {{ $soldering->location }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="text-sm text-zinc-700 dark:text-zinc-300 uppercase">
                                        {{ strtoupper($soldering->type ?? '-') }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="text-sm text-zinc-700 dark:text-zinc-300 uppercase">
                                        {{ strtoupper($soldering->spec ?? '-') }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm font-semibold text-zinc-800 dark:text-white">
                                            {{ $soldering->solderingDetails()->count() }}
                                        </span>
                                ...
                                <td class="px-4 py-3">
                                    <div class="text-sm min-w-0">
                                        <div class="truncate max-w-[150px]" title="{{ $soldering->creator->name ?? 'N/A' }}">
                                            {{ $soldering->creator->name ?? 'N/A' }}
                                        </div>
                                        <div class="text-xs text-zinc-500 whitespace-nowrap">
                                            {{ $soldering->created_at->format('d M Y') }}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1 whitespace-nowrap">
                                        @can('view soldering')
                                        <flux:button 
                                            href="{{ route('esd.solderings.show', $soldering->id) }}"
                                            wire:navigate
                                            size="sm"
                                            icon="eye"
                                            variant="primary"
                                            ...
                                            title="View soldering details"
                                        />
                                        @endcan
                                        @can('edit soldering')
                                        <flux:button 
                                            wire:click="edit({{ $soldering->id }})" 
                                            x-on:click="$dispatch('open-modal', 'soldering-form-modal')"
                                            size="sm"
                                            icon="pencil-square"
                                            variant="primary"
                                            color="yellow"
                                            class="!p-2 flex-shrink-0"
                                            title="Edit soldering"
                                        />
                                        @endcan

                                        @can('delete soldering')
                                            <flux:button 
                                                wire:click="confirmDelete({{ $soldering->id }})" 
                                                x-on:click="$dispatch('open-modal', 'delete-soldering-modal')"
                                                size="sm"
                                                icon="trash"
                                                variant="primary"
                                                color="red"
                                                class="!p-2 flex-shrink-0"
                                                title="Delete soldering"
                                            />
                                        @endcan
                                    </div>
                                </td>
                                    ...
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
                                                {{ $search || $filterArea || $filterLocation || $filterType || $filterLine ? 'Try adjusting your filters' : 'Get started by creating a new soldering record' }}
                                            </p>
                                        </div>
                                        @if($search || $filterArea || $filterLocation || $filterType || $filterLine)
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" wire:model="spec" 
                                            oninput="this.value = this.value.toUpperCase()"
                                            class="w-full px-3 py-2 border rounded-lg dark:bg-zinc-800 dark:border-zinc-700 uppercase">
                                        @error('spec') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        @error('line') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                    </div>
                ...
                <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-xl w-full max-w-md p-6 text-center">
                        ...
                        <p class="text-gray-600 dark:text-gray-400 mb-6">
                            Are you sure you want to delete soldering "{{ $solderingToDelete?->register_no }}"? This action cannot be undone.
                        </p>
