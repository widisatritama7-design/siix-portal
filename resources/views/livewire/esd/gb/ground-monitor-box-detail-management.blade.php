{{-- resources/views/livewire/esd/gb/ground-monitor-box-detail-management.blade.php --}}
<section class="w-full">
    @include('partials.esd-heading')

    <flux:heading class="sr-only">
        {{ __('Electrostatic Discharge - Ground Monitor Box Measurement') }}
    </flux:heading>
            ...
            <div x-data="{ showFilters: false }">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-1 mb-4">
                    ...
                    <div class="flex gap-2">
                        @if($filterGroundMonitorBox || $filterArea || $filterLocation || $filterG3 || $filterG4 || $filterDateFrom || $filterDateUntil || $filterNextDateFrom || $filterNextDateUntil || $search)
                ...
                <div x-show="showFilters" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select wire:model.live="filterGroundMonitorBox" class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white">
                                <option value="">All Register</option>
                                @foreach($groundMonitorBoxes as $groundMonitorBox)
                                    <option value="{{ $groundMonitorBox->id }}">{{ $groundMonitorBox->register_no }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <!-- G1 (g_3) Filter -->
                        <div>
                            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">G1</label>
                            <select wire:model.live="filterG3" class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white">
                        ...
                        <!-- G2 (g_4) Filter -->
                        <div>
                            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">G2</label>
                            <select wire:model.live="filterG4" class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white">
                                <option value="">All</option>
                                <option value="YES">YES</option>
                                <option value="NO">NO</option>
                            </select>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center text-white font-medium shadow-lg flex-shrink-0 text-xs">
                                            {{ strtoupper(substr($detail->groundMonitorBox->register_no ?? '', 0, 1)) }}
                                        </div>
                                        <span class="text-sm font-semibold text-zinc-800 dark:text-white truncate max-w-[180px]" title="{{ $detail->groundMonitorBox->register_no ?? '' }}">
                                            {{ $detail->groundMonitorBox->register_no ?? 'N/A' }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300">
                                    {{ $detail->groundMonitorBox->area ?? 'N/A' }}
                                </td>
                                <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300">
                                    {{ $detail->groundMonitorBox->location ?? 'N/A' }}
                                </td>
                                <td class="px-4 py-3">
                                    @php
                                    ...
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
                                                {{ $search || $filterGroundMonitorBox || $filterArea || $filterLocation ? 'Try adjusting your filters' : 'Get started by adding a new measurement' }}
                                            </p>
                                        </div>
                                        @if($search || $filterGroundMonitorBox || $filterArea || $filterLocation)
                            ...
                            <form wire:submit="save">
                                ...
                                <div class="mb-4">
                                    ...
                                    <select wire:model="ground_monitor_box_id"
                                        ...
                                        @foreach($groundMonitorBoxes as $groundMonitorBox)
                                            <option value="{{ $groundMonitorBox->id }}">{{ $groundMonitorBox->register_no }} - {{ $groundMonitorBox->area }} - {{ $groundMonitorBox->location }}</option>
                                        @endforeach
                                    </select>
                                    @error('ground_monitor_box_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
