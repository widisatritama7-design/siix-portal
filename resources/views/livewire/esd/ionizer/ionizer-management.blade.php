        ...
        <div class="-mt-2">
            ...
            <div class="flex flex-col sm:flex-row gap-3 mt-2 mb-6">
                ...
                <div class="w-full sm:w-[20%]">
                    <flux:button 
                        href="{{ route('esd.ionizer-details')}}"
                        wire:navigate
                        icon="arrow-right"
                        variant="primary"
                        color="green"
                ...
                <div class="w-full sm:w-[20%]">
                    ...
                    <flux:button 
                        ...
                        x-on:click="$dispatch('open-modal', 'ionizer-form-modal')"
                    >
                        Add New
                    </flux:button>
                    @endcan
                ...
                <div class="overflow-x-auto">
                    <table class="w-full whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                            @forelse($ionizers as $index => $ionizer)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors" wire:key="ionizer-{{ $ionizer->id }}">
                                <td class="px-4 py-3 text-sm text-zinc-500 dark:text-zinc-400">
                                    {{ $ionizers->firstItem() + $index }}
                                 </>
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center text-white font-medium shadow-lg flex-shrink-0">
                                            {{ strtoupper(substr($ionizer->register_no, 0, 1)) }}
                                        </div>
                                        <div class="min-w-0">
                                            <span class="text-sm font-semibold text-zinc-800 dark:text-white block truncate max-w-[200px]" title="{{ $ionizer->register_no }}">
                                                {{ $ionizer->register_no }}
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <td class="px-4 py-3">
                                    <span class="text-sm text-zinc-700 dark:text-zinc-300 block truncate max-w-[150px]" title="{{ $ionizer->area }}">
                                        {{ $ionizer->area }}
                                    </span>
                                 </>
                                <td class="px-4 py-3">
                                    <span class="text-sm text-zinc-700 dark:text-zinc-300 block truncate max-w-[150px]" title="{{ $ionizer->location }}">
                                        {{ $ionizer->location }}
                                    </span>
                                 </>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <div class="text-sm min-w-0">
                                        <div class="truncate max-w-[150px]" title="{{ $ionizer->creator->name ?? 'N/A' }}">
                                            {{ $ionizer->creator->name ?? 'N/A' }}
                                        </div>
                                        <div class="text-xs text-zinc-500 whitespace-nowrap">
                                            {{ $ionizer->created_at->format('d M Y') }}
                                        </div>
                                    </div>
                                 </>
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1 whitespace-nowrap">
                                        @can('view ionizer')
                                        <flux:button 
                                            href="{{ route('esd.ionizers.show', $ionizer->id) }}"
                                            wire:navigate
                                            size="sm"
                                            icon="eye"
                                            variant="primary"
                                            ...
                                            title="View ionizer details"
                                        />
                                        @endcan
                                        @can('edit ionizer')
                                        <flux:button 
                                            wire:click="edit({{ $ionizer->id }})" 
                                            x-on:click="$dispatch('open-modal', 'ionizer-form-modal')"
                                            size="sm"
                                            icon="pencil-square"
                                            variant="primary"
                                        ...
                                        @can('delete ionizer')
                                            <flux:button 
                                                wire:click="confirmDelete({{ $ionizer->id }})" 
                                                x-on:click="$dispatch('open-modal', 'delete-ionizer-modal')"
                                                size="sm"
                                                icon="trash"
                                                variant="primary"
                                                color="red"
                                                class="!p-2 flex-shrink-0"
                                                title="Delete ionizer"
                                            />
                                        @endcan
                                    </div>
                                 </>
                                    ...
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
                                                {{ $search ? 'Try adjusting your search query' : 'Get started by creating a new ionizer record' }}
                                            </p>
                                        </div>
                                        @if($search)
                            ...
                            <form wire:submit="save">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" 
                                           wire:model="gap"
                                           class="w-full px-3 py-2 border rounded-lg dark:bg-zinc-800 dark:border-zinc-700 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                    @error('gap') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
                ...
                <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-xl w-full max-w-md p-6 text-center">
                        ...
                        <p class="text-gray-600 dark:text-gray-400 mb-6">
                            Are you sure you want to delete ionizer "{{ $ionizerToDelete?->register_no }}"? This action cannot be undone.
                        </p>
