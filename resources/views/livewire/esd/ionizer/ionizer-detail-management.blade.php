{{-- resources/views/livewire/esd/ionizer/ionizer-detail-management.blade.php --}}
<section class="w-full">
    @include('partials.esd-heading')

    <flux:heading class="sr-only">
        {{ __('Electrostatic Discharge - Ionizer Measurement') }}
    </flux:heading>
        ...
        <div class="-mt-2">
            ...
            <div x-data="{ showFilters: false }">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-1 mb-4">
                    ...
                    <div class="flex gap-2">
                        @if($filterIonizer || $filterArea || $filterLocation || $filterDateFrom || $filterDateUntil || $filterNextDateFrom || $filterNextDateUntil || $search)
            ...
            <div class="mb-4">
                <div class="bg-white dark:bg-zinc-800 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700 shadow-sm">
                    ...
                    @if($printRegisterNo || $printArea || $printLocation || $printDateFrom || $printDateUntil)
                        ...
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                            ...
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors" wire:key="detail-{{ $detail->id }}">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center text-white font-medium shadow-lg flex-shrink-0 text-xs">
                                            {{ strtoupper(substr($detail->ionizer->register_no ?? '', 0, 1)) }}
                                        </div>
                                        <span class="text-sm font-semibold text-zinc-800 dark:text-white truncate max-w-[180px]" title="{{ $detail->ionizer->register_no ?? '' }}">
                                            {{ $detail->ionizer->register_no ?? 'N/A' }}
                                        </span>
                                    </div>
                                </>
                                <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300 truncate max-w-[200px]" title="{{ $detail->ionizer->area ?? '' }}">
                                    {{ $detail->ionizer->area ?? 'N/A' }}
                                </>
                                <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300">
                                    {{ $detail->ionizer->location ?? 'N/A' }}
                                </>
                                <td class="px-4 py-3">
                                    @php
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $pm1Color }}">
                                        {{ $detail->pm_1 }}
                                    </span>
                                </>
                                <td class="px-4 py-3">
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $pm2Color }}">
                                        {{ $detail->pm_2 }}
                                    </span>
                                </>
                                <td class="px-4 py-3">
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $pm3Color }}">
                                        {{ $detail->pm_3 }}
                                    </span>
                                </>
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $judgeColor }}">
                                    ...
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
                                                {{ $search || $filterIonizer || $filterArea || $filterLocation ? 'Try adjusting your filters' : 'Get started by adding a new measurement' }}
                                            </p>
                                        </div>
                                        @if($search || $filterIonizer || $filterArea || $filterLocation)
                                ...
                                <div class="mb-4">
                                    ...
                                    <select wire:model="ionizer_id"
                                        ...
                                        @foreach($ionizers as $ionizer)
                                            <option value="{{ $ionizer->id }}">{{ $ionizer->register_no }} - {{ $ionizer->area }} - {{ $ionizer->location }}</option>
                                        @endforeach
                                    </select>
                                    @error('ionizer_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="mt-1">
                                                <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {{ $judgement_c1_before == 'OK' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                    Judgement: {{ $judgement_c1_before ?: 'Auto' }}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="mt-1">
                                                <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {{ $judgement_c2_before == 'OK' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                    Judgement: {{ $judgement_c2_before ?: 'Auto' }}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="mt-1">
                                                <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {{ $judgement_c3_before == 'OK' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                    Judgement: {{ $judgement_c3_before ?: 'Auto' }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="mb-6 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="mt-1">
                                                <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {{ $judgement_c1 == 'OK' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                    Judgement: {{ $judgement_c1 ?: 'Auto' }}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="mt-1">
                                                <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {{ $judgement_c2 == 'OK' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                    Judgement: {{ $judgement_c2 ?: 'Auto' }}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="mt-1">
                                                <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {{ $judgement_c3 == 'OK' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                    Judgement: {{ $judgement_c3 ?: 'Auto' }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-xl w-full max-w-md p-6 text-center">
                        ...
                        <p class="text-gray-600 dark:text-gray-400 mb-6">
                            Are you sure you want to delete measurement for "{{ $detailToDelete?->ionizer?->register_no ?? 'this ionizer' }}"? This action cannot be undone.
                        </p>
