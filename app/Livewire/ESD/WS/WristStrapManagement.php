...
class WristStrapManagement extends Component
{
    ...
    public function printPDF()
    {
        ...
        if (empty($this->printRegisterFilter) && empty($this->printTypeFilter) && empty($this->printJudgementFilter) && empty($this->printDateFrom) && empty($this->printDateUntil)) {
            $this->dispatch('notify', message: 'Please select at least one filter (Register No, Type, Judgement, or Date Range)!', type: 'error');
            return;
        }

        // Query data untuk print
        $query = WristStrap::with('creator');
        ...
        // Filter by Type
        if (!empty($this->printTypeFilter)) {
            $query->where('type', 'like', '%' . $this->printTypeFilter . '%');
        }
        ...
        // Generate PDF
        $pdf = Pdf::loadView('livewire.esd.ws.wrist-strap-pdf', $data);
        ...
        return response()->streamDownload(
            ...
            'wrist-strap-' . Carbon::now()->format('Ymd_His') . '.pdf'
        );
    }
