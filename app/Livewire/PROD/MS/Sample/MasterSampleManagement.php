...
class MasterSampleManagement extends Component
{
    ...
    {
        // When page changes, update selectAll checkbox status for new page
        $currentPage = $this->getPage();
        $this->selectAll = in_array($currentPage, $this->selectedPages);
    }
    ...
    public function updatedSelectAll($value)
    {
        ...
        if ($value) {
            ...
            $currentPageIds = $this->getMasterSamplesQuery()
                ->orderBy('id', 'asc')
                ->paginate(10)
                ->getCollection()
                ->pluck('id')
            ...
            $this->selectedSamples = array_unique(array_merge($this->selectedSamples, $currentPageIds));
        } else {
            // Remove current page from selected pages
            $this->selectedPages = array_diff($this->selectedPages, [$currentPage]);
            
            // Remove current page IDs from selected samples
            $currentPageIds = $this->getMasterSamplesQuery()
                ->orderBy('id', 'asc')
                ->paginate(10)
                ->getCollection()
            ...
            $this->selectedSamples = array_diff($this->selectedSamples, $currentPageIds);
        }
        
        // Reindex array
        $this->selectedSamples = array_values($this->selectedSamples);
    }

    public function getMasterSamplesQuery()
    {
        $query = MasterSample::query();
        
        // Apply tab filter
        $now = now()->startOfDay();
        switch ($this->activeTab) {
            case 'active':
                $query->whereHas('details', fn($q) => $q->where('expired_date', '>', $now));
                break;
            case 'expired':
                $query->where('status', 'ACTIVE')
                    ->whereHas('details', fn($q) => $q->where('expired_date', '<=', $now));
                break;
            case 'eol':
            ...
            case 'expire_this_month':
                $query->where('status', 'ACTIVE')
                    ->whereHas('details', fn($q) => $q->whereBetween('expired_date', [
                        $now->copy()->startOfMonth(),
                        $now->copy()->endOfMonth(),
                    ]));
                break;
        ...
        ->when($this->filterCustomer, fn($q) => $q->where('customer', $this->filterCustomer))
        ->when($this->filterNameOrMc, fn($q) => $q->where('name_or_mc', $this->filterNameOrMc));
    ...
    public function bulkPrintLabel()
    {
        if (empty($this->selectedSamples)) {
            $this->dispatch('notify', message: 'Please select at least one sample to print!', type: 'error');
            return;
        }
        
        $ids = implode(',', $this->selectedSamples);
        
        return redirect()->to("/prod/master-sample/print/{$ids}");
    }
    ...
    private function prepareDetailData($detailData)
    {
        $data = [
            ...
            'created_by' => auth()->id(),
            'updated_by' => auth()->id(),
            // Semua date fields di-set NULL saat create
            'check_date' => null,
            'knowladge_date' => null,
            'approve_date' => null,
        ];
    ...
    public function render()
    {
        ...
        // Gunakan getMasterSamplesQuery() untuk mendapatkan query yang sudah difilter
        $query = $this->getMasterSamplesQuery();
        $masterSamples = $query->orderBy('id', 'asc')->paginate(10);
        ...
        return view('livewire.prod.ms.sample.master-sample-management', [
